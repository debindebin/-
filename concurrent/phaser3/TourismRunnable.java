...
public class TourismRunnable implements  Runnable{
    ...
    private boolean goToPoint(String point){
        try {
            ...
            System.out.println(name + " 花了 " + getRandomTime() + " 时间才到了" + point+"------"+phaser.getPhase());
    ...
    private boolean randomEvent() {
        ...
        if (r < 10){
            int friendNum =  1;
            System.out.println(name + ":在这里竟然遇到了"+friendNum+"个朋友,他们说要一起去旅游..."+"------"+phaser.getPhase());
            phaser.bulkRegister(friendNum);
            for (int i = 0; i < friendNum; i ++){
                new Thread(new TourismRunnable(phaser), name + "的朋友" + frientCount.getAndAdd(1) + "号").start();
            }
        }else if(r > 90){
            System.out.println(name + ":突然有事要离开一下,不和他们继续旅游了...."+"------"+phaser.getPhase());
